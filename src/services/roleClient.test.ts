...
describe('validateRoleNameUniqueness', () => {
  ...
  it('should return error on database failure', async () => {
    ;(roleNameExists as any).mockRejectedValue(new Error('Database connection failed'))
...
describe('createCustomRole', () => {
  ...
  it('should create a role with valid inputs', async () => {
    ...
    const result = await createCustomRole('Engineering', 'Engineering team', 'user-1')

    expect(result).toEqual(mockRole)
    expect(dbCreateRole).toHaveBeenCalledWith('Engineering', 'Engineering team', 'user-1')
  })
  ...
  it('should reject duplicate role names', async () => {
    ...
    ;(dbCreateRole as any).mockRejectedValue(
      new Error('A role with name "Engineering" already exists')
    )
...
describe('deleteCustomRole', () => {
  ...
  it('should reject deletion of role in use', async () => {
    ...
    await expect(promise).rejects.toThrow('Cannot delete this role because it is in use')
