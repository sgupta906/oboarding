...
| 57 | `users-panel-signin-bug` | 2026-02-20 | 46d6aa3 | Fixed bug #35 (P3 LOW) - users created via Manager Dashboard > Users panel were persisted to Supabase users table but signInWithEmailLink() only checked localStorage for credentials. Added Step 1.5 to sign-in flow querying Supabase users table (joined with user_roles) as fallback when no localStorage credential exists. Users found with at least one role are signed in as 'manager' and credential cached in localStorage. Added getUserByEmail() to userService.ts. +12 tests (710 total) |
